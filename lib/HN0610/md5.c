...
void md5_hash(uint8_t md5_pass[16], const uint8_t *input, uint32_t length, uint8_t output[16]) 
{
    uint32_t hash[4];
    uint32_t block[16];
    uint32_t i, key;
    ...
    for (i = 0; i < 4; i++)
    {
		key = (((uint32_t)(md5_pass[i*4 + 0] & 0xff) << 0)| \
		       ((uint32_t)(md5_pass[i*4 + 1] & 0xff) << 8)| \
		       ((uint32_t)(md5_pass[i*4 + 2] & 0xff) << 16)| \
		       ((uint32_t)(md5_pass[i*4 + 3] & 0xff) << 24));
        hash[i] = key;
    }
	
    for (i = 0; length - i >= 64; i += 64)
        md5_compress(hash, (uint32_t *)(input + i));  // Type-punning
	
	byteBlock = (uint8_t *)block;  // Type-punning
	
    rem = length - i;
    memcpy(byteBlock, input + i, rem);
    ...
    block[15] = length >> 29;
	md5_compress(hash, block);
    memcpy((void*)output, (void*)hash, 16);
}
